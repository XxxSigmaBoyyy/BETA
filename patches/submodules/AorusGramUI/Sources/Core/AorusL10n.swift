...
public struct AorusL10n {
    ...
    public var userVPNFooter: String {
        t(
            "Добавьте ключ VLESS, VMess, Trojan, Shadowsocks, Hysteria2 или ссылку на подписку из буфера обмена. Параметры подключения доступны внутри конфигурации.",
            "Add a VLESS, VMess, Trojan, Shadowsocks or Hysteria2 key or a subscription link from the clipboard. Connection options are available inside the configuration."
        )
    } // AORUS-VPN
    ...
    public var userVPNImportUnsupported: String { t("Поддерживаются ключи VLESS, VMess, Trojan, Shadowsocks, Hysteria2 и ссылки на подписку.", "VLESS, VMess, Trojan, Shadowsocks and Hysteria2 keys and subscription links are supported.") } // AORUS-VPN
