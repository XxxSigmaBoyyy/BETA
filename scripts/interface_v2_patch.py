...
def _patch_avatar_tint_publish(tg: Path) -> None:
    ...
    if "publishAvatarTint" in text:
        ...
        text = _replace_once(
            ...
            "        // has always had.\n"
            "        // The banner has to be visible to be worth reading. Its alpha goes to zero\n"
            "        // once the page is scrolled far enough for the header to hand over to the\n"
            "        // navigation bar, and a read taken then returns the backdrop alone -- while\n"
            "        // still spending one of the five the settle loop is allowed. Five of those in\n"
            "        // a row and the key is marked settled with the wrong colour for good. Skipped\n"
            "        // rather than refused inside the sampler, so no reading is spent at all: this\n"
            "        // runs again from the next layout pass, and scrolling causes those.\n"
            "        if listContainerNode.galleryEntries.isEmpty,\n"
            "           peer.smallProfileImage == nil,\n"
            "           peer.effectiveProfileColor != nil,\n"
            "           self.backgroundBannerView.alpha > 0.01,\n"
            "           !self.backgroundBannerView.isHidden {\n"
            "            // The banner rather than the cover component's own view. The banner is a\n"
            "            // stored property that exists from init and is the thing the header\n"
            "            // actually shows -- the cover is a `ComponentView` whose view may not be\n"
            "            // built, or may not have a size, on the pass that gets here, and it is not\n"
            "            // guaranteed to be opaque by itself. Reading the banner reads whatever is\n"
            "            // on screen, which is the only definition of the colour worth continuing.\n"
            "            //\n"
            "            // Its height is deliberately not checked. A view that has nothing to give\n"
            "            // yet makes the sampler return nil, and nil is what schedules the re-read\n"
            "            // loop -- the same loop a photo still decoding goes through. Refusing the\n"
            "            // call here instead would mean no loop and no second chance, because a\n"
            "            // profile sitting still gets no further layout passes.\n"
            "            AorusGlassProfileTint.publishAvatarTint(\n"
            "                for: peer.id.id._internalGetInt64Value(),\n"
            "                photo: 0,\n"
            "                photoCount: 0,\n"
            "                view: self.backgroundBannerView,\n"
            "                mirroredTail: 0.0,\n"
            "                isFullPhoto: true,\n"
            "                // Read as composited over the page, and without the block shadow.\n"
            "                //\n"
            "                // A cover for a peer with no `profileColor` fills itself with `.clear`\n"
            "                // and paints only its pattern, so the colour shown there is the page\n"
            "                // coming through it; reading that layer alone samples a pattern over\n"
            "                // nothing and the coverage test refuses it.\n"
            "                //\n"
            "                // And no shadow to take off: that correction exists for the block\n"
            "                // Telegram draws over a photograph\'s last row, which belongs to the\n"
            "                // avatar gallery. A peer with no photos has no gallery and no block, so\n"
            "                // subtracting it is what left the page a visible step darker than the\n"
            "                // header. The photo path keeps the correction exactly as it was.\n"
            "                mode: .composited(over: self.presentationData?.theme.list.blocksBackgroundColor),\n"
            "                onUpdate: { [weak self] in\n"
            "                    self?.requestUpdateLayout?(false)\n"
            "                }\n"
            "            )\n"
