...
def _patch_avatar_tint_publish(tg: Path) -> None:
    ...
    if "publishAvatarTint" in text:
        ...
        text = _replace_once(
            ...
            "                mirroredTail: 0.0,\n"
            "                isFullPhoto: true,\n"
            "                // The page laid down under the banner before it is read. A cover for a\n"
            "                // peer with no `profileColor` fills itself with `.clear` and paints only\n"
            "                // its pattern, so the colour shown there is the page coming through it;\n"
            "                // reading the layer alone samples a pattern over nothing and the\n"
            "                // coverage test refuses it. With the page underneath, the read is the\n"
            "                // header as composited -- which is the colour to continue.\n"
            "                backdrop: self.presentationData?.theme.list.blocksBackgroundColor,\n"
            "                onUpdate: { [weak self] in\n"
            "                    self?.requestUpdateLayout?(false)\n"
            "                }\n"
            "            )\n"
